...
from keras.layers.core import Activation, Dense, RepeatVector, Dropout
...
def load_inputs(input_file, map=None):
    ...
    header_len = 2
    lines = []
    with open(("/Users/admin/Downloads/Context2Name-master/" + input_file), "r") as file:  #パス名追加
        for line in file:
            line = line.rstrip()  #最後の空白を削除
            lines.append(line)
    j = 0
    for line in lines:
        tokens = line.split()  #csvの1行を空白で分割
...
def create_lstm(output_vocab_size):
    ...
    layer1 = LSTM(config.HIDDEN_LAYER_SIZE2, return_sequences=False, dropout = 0.1, recurrent_dropout = 0.1)(inputs)
